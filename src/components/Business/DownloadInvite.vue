<template>
  ...
  >
    <div class="share-box">
      <div>{{ useUserStoreHook().$state.inviteInfo.invite_code }}</div>
      <el-icon
        style="cursor: pointer"
        @click="onInvite(useUserStoreHook().$state.inviteInfo.invite_code)"
        ><CopyDocument
      /></el-icon>
    </div>
    <p class="share-text">*您可以分享邀请码给好友，好友注册时填写邀请码</p>
    <p class="share-title">下载链接</p>
    <div class="share-box">
      <div>{{ useUserStoreHook().$state.inviteInfo.share_text }}</div>
      <el-icon
        style="cursor: pointer"
        @click="onInvite(useUserStoreHook().$state.inviteInfo.share_text)"
        ><CopyDocument
      /></el-icon>
    </div>
    <p class="share-text">*您可以分享给好友，好友进行下载</p>
  </el-dialog>
</template>
...
const handleClose = () => {
  useUserStoreHook().openDownloadInviteDlg(false);
};
