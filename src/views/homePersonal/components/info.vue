    ...
    <div :class="[ns.b('content'), 'animate__animated animate__fadeIn']">
      <div :class="[ns.be('content', 'left')]">
        ...
        >
          ...
          <el-switch
            v-model="showInfo.weChat"
            @change="
              (val) => {
                return onChangeSwitch(val, 'weChat');
              }
            "
          />
        </div>
      ...
      <div :class="[ns.be('content', 'right')]">
        ...
        <div>
          ...
          <h5
            v-if="showInfo.mobile"
            :class="[
              ns.be('right', 'common'),
              'animate__animated animate__fadeIn',
            ]"
          >
            手机：{{ modifyInfoFreeze.mobile }}
          </h5>
          <h5
            v-if="showInfo.weChat"
            :class="[
              ns.be('right', 'common'),
              'animate__animated animate__fadeIn',
            ]"
          >
            微信：{{ modifyInfoFreeze.weCat }}
          </h5>
          <h5
            v-if="showInfo.email"
            :class="[
...
const onGetRegionInfo = computed(() => {
  ...
  if (info === undefined || info.region === null) {
    ...
    if (info.region.subRegion.subRegion.subRegion.subRegion === null) {
      _data =
        info.region.subRegion.subRegion.name +
        "/" +
        info.region.subRegion.subRegion.subRegion.name;
    } else {
      _data =
        info.region.subRegion.subRegion.name +
        "/" +
        info.region.subRegion.subRegion.subRegion.name +
        "/" +
        info.region.subRegion.subRegion.subRegion.subRegion.name;
    }
  }
  return _data;
});
