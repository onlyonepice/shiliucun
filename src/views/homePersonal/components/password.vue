<template>
  <div :class="[ns.b()]">
    ...
    <div :class="[ns.b('content'),'animate__animated animate__fadeIn']">
      <div :class="ns.b('head')">
        <div :class="[ step === 1 ? ns.b('chose') : '', step === 2 ? ns.bm('chose','has') : '']">获取验证码</div>
        <div :class="[ step === 2 ? ns.b('chose') : '' ]">修改密码</div>
      </div>
      <template v-if="step === 1">
        <p :class="ns.b('text')">手机号码</p>
        <Select type="input" style="width: 100%;" :defaultValue='mbInfo.mobile' :disabled="true" @onChange="val=>{ return onChangeInfo(val,'mobile') }" />
        <p :class="ns.b('text')">验证码</p>
        <div :class="ns.be('step1','content')">
          <Select type="input" style="width: 100%;" :defaultValue='mbInfo.code' @onChange="val=>{ return onChangeInfo(val,'code') }" />
          <p type="primary" :class="[ns.b('btn'),btnDesc.indexOf('s') !== -1 ? ns.bm('btn','down') : '']" @click="onGetCode()">{{ btnDesc }}</p>
        </div>
        <el-button type="primary" :class="ns.b('next')" @click="onNext()">下一步</el-button>
      </template>
      <template v-else>
        <p :class="ns.b('text')">新密码</p>
        <div :class="ns.b('special')">
          <img :src="PasswordKey" alt="">
          <Select type="input" style="width: 100%;" :specialType="showPassword.password1 ? 'password' : 'text'" @onChange="val=>{ return onChangePassword(val,'password1') }" />
          <img :src="!showPassword.password1 ? PasswordHidden : PasswordShow" @click="onChangeShow('password1')" alt="">
        </div>
        <p :class="ns.b('text')">再次输入新密码</p>
        <div :class="ns.b('special')">
          <img :src="PasswordKey" alt="">
          <Select type="input" style="width: 100%;" :specialType="showPassword.password2 ? 'password' : 'text'" @onChange="val=>{ return onChangePassword(val,'password2') }" />
          <img :src="!showPassword.password2 ? PasswordHidden : PasswordShow" @click="onChangeShow('password2')" alt="">
        </div>
        <el-button type="primary" :class="ns.b('next')" @click="onConfirm()">完成</el-button>
      </template>
    </div>
  </div>
</template>
...
import { modifyPasswordCode, checkPasswordCode, modifyPasswordApi } from '@/api/user'
...
const onConfirm = async () => {
  ...
  const { resp_code }:any = await modifyPasswordApi({ password: modifyPassword.value.password1, smsCode: mbInfo.value.code })
  if( resp_code === 0 ){
