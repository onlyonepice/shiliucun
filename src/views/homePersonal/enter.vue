      ...
      <div :class="ns.b('box')">
        <div :class="ns.be('box','left')">
          ...
          <div :class="ns.bm('left','list')">
            <h5>系列产品</h5>
            <div :class="ns.bm('left','item')" v-for="item in seriesList" :key="item.id" @click="onToPage(item.path)">
              <img :src="item.icon" alt="">
              <div>
                <h5>{{ item.title }}</h5>
                <p v-if="!item.time" :class="ns.bm('item','tab')">待体验</p>
                <p v-else :class="ns.bm('item','time')">上次使用时间：{{ item.time }}</p>
              </div>
            </div>
          </div>
        </div>
...
const seriesList: Ref<Array<any>> = ref([
  { id: 1, path: VITE_INDUSTRIALMAP_URL, icon: MapIcon, title: '产业链地图', time: '' },
  { id: 2, path: VITE_CLASSROOM_URL, icon: ClassRoomIcon, title: '储能云课堂', time: '' }
])
