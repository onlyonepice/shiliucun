<template>
  <div id="investment-return" class="page">
    <h5 class="page__title">工商业投资回报性</h5>
    ...
    <div v-if="filterFinish">
      ...
      <div class="filter__content investment-filter__check">
        <Select width="30%" title="年利用天数" :options="yearList" valueKey="paramName" labelKey="paramDesc"
          :defaultValue="searchParams.annualDays" @onChange="$event => { onChangeFilter($event, 'annualDays') }" />
        <Select width="30%" title="贴现率" :options="rateList" valueKey="paramName" labelKey="paramDesc"
          :defaultValue="searchParams.bankRate" @onChange="$event => { onChangeFilter($event, 'bankRate') }" />
        <Select width="30%" title="测算周期" :options="periodList" valueKey="paramName" labelKey="paramDesc"
          :defaultValue="searchParams.calculationPeriod"
          @onChange="$event => { onChangeFilter($event, 'calculationPeriod') }" />
        <Select v-if="showInfoList[0][0].value === 'EMC合同能源'" width="30%" title="业主分成" type="number"
          :defaultValue="searchParams.ownersShare" @onChange="$event => { onChangeFilter($event, 'ownersShare') }" />
        <Select v-if="showInfoList[0][0].value === 'EMC合同能源'" width="30%" title="投资方分成" type="number"
          :defaultValue="searchParams.dividedByInvestors"
          @onChange="$event => { onChangeFilter($event, 'dividedByInvestors') }" />
      </div>
        ...
        <el-table :data="revenueEstimateList" style="width: 100%" header-row-class-name="table-class">
          ...
          <el-table-column prop="financialCost" label="现金流">
            <template #default="scope">
              <span :class="{ 'color-red': scope.row.financialCost.indexOf('-') > -1 }">{{ scope.row.
                financialCost }}</span>
            </template>
          </el-table-column>
      ...
      <el-table :data="dischargeList" style="width: 100%" header-row-class-name="table-class" empty-text="暂无数据">
        ...
        <el-table-column prop="annualDischarge" label="年放电量">
          <template #default="scope">
            <span v-if="scope.row.annualDischarge" :class="{ 'color-red': scope.row.annualDischarge.indexOf('-') > -1 }">{{
