
<template>
    <div class="content">
      <el-carousel class="banner" motion-blur :interval="5000" >
        <el-carousel-item v-for="item in 4" :key="item" >
          <h3 class="small justify-center" text="2xl">{{ item }}</h3>
        </el-carousel-item>
      </el-carousel>
      <div class="card_block">
        <div class="title">
          <div clas=text>【全球首款】真人性愛模擬器</div>
        </div>
        <div class="item_block">
          <div class="item" v-for="item in fileList" :key="item" @click="goto">
            <img class="card_img" :src="test"/>
            <div class="bottom" > 
               <div class="tips">
                  涩点点《超色气女仆 永井すみれ》
                </div>
                <div class="coupon">
                  
                </div>
                <div class="price">
                  免费
                </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </template>
  
  <script lang="ts" setup>
  import { ref } from "vue";
  import { useRouter } from 'vue-router';
  import test from "@/assets/img/test.jpg";
  const router = useRouter();
  const goto = ()=>{
    router.push({path:"/gameItem"})
  }
  const fileList = ref([1,2,3,4,5,6,7,8])
  </script>
  
  <style lang="scss" scoped>
  @import "@/style/mixin.scss";
  ::-webkit-scrollbar {
    display: none;
  }
  
  .content{
    width:100%;
    display: flex;
    flex-direction: column;
    align-items: center;
  }
  .card_block{
    margin-top:1.5vw;
    width: 100%;
    background: crimson;
    width:calc(100% - 100px);
    border-radius: 6px;
    padding:var(--xs, .625vw) var(--lg, 1.25vw);
    background: var(--neutral-950,#171616);
    .item_block{
      display: flex;
      flex-direction: row;
      grid-gap: .625vw;
      gap: .625vw;
      grid-gap: var(--xs, .625vw);
      gap: var(--xs, .625vw);
      overflow-x: auto;
  
    }
    .title{
      font-size: 1.45833vw;
      font-weight: 400;
      background-clip: text;
      background-image: linear-gradient(180deg, #bcb1b1, #786b6b);
      color: transparent;
      margin-bottom: var(--sm, .83333vw);
    }
    .bottom{
      padding: var(--xs, .625vw) 0 var(--3xs, .41667vw);
    }
    .text{
      background: var(--neutral-950,#171616)
    }
    .item{
      &:hover{
        border:.5px solid crimson;
        background-image: url("@/assets/img/card-select.png");
      }
      cursor: pointer;
     
      width: 14.16667vw;
      padding: var(--3xs, .41667vw);
      position: relative;
      border-radius: .625vw;
      border-radius: var(--md, .625vw);
      cursor: pointer;
      background-color: var(--neutral-900, #222121);
      padding: 1.25vw;
      background-image: url("@/assets/img/card.png");
      background-position: 100% 100%;
      background-size: 200%;
      background-repeat: no-repeat;
      .tips{
        height: var(--xl, 1.66667vw);
        overflow: hidden;
        color: #f0eded;
        color: var(--neutral-50, #f0eded);
        text-overflow: ellipsis;
        white-space: nowrap;
        font-size: .72917vw;
        font-weight: 400;
        line-height: .83333vw;
        height: var(--xl, 1.66667vw);
      }
      .coupon{
        display: flex;
        flex-direction: column;
        align-items: flex-start;
        color: #f00a38;
        color: var(--primary-300, #f00a38);
        font-size: .83333vw;
        font-weight: 400;
        line-height: 1.04167vw;
        grid-gap: .20833vw;
        gap: .20833vw;
        }
      .price{
        margin-top:2vw;
        display: flex;
        flex-direction: column;
        align-items: flex-start;
        color: #f00a38;
        color: var(--primary-300, #f00a38);
        font-size: .83333vw;
        font-weight: 400;
        line-height: 1.04167vw;
        grid-gap: .20833vw;
        gap: .20833vw;
        }
    }
  }
  .banner{
    width:calc(100% - 100px);
    height:0;
    padding-bottom:calc((120% - 10px)/3);
    border-radius: 6px;
    
  }
  .card_img{
      width: 100%;
      height: 9.58333vw;
      border-radius: .41667vw;
      border-radius: var(--3xs, .41667vw);
      object-fit: cover;
  }
  .demonstration {
    color: var(--el-text-color-secondary);
  }
  
  .el-carousel__item h3 {
    color: #475669;
    opacity: 0.75;
    margin: 0;
    text-align: center;
  }
  
  .el-carousel__item:nth-child(2n) {
    background-color: #99a9bf;
  }
  
  .el-carousel__item:nth-child(2n + 1) {
    background-color: #d3dce6;
  }
  </style>
  